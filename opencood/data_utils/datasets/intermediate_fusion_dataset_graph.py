...
class IntermediateFusionDatasetGraph(basedataset.BaseDataset):
    """
    This class is for intermediate fusion where each vehicle transmit the
    deep features to ego.
    """
    def __init__(self, params, visualize, train=True):
        super(IntermediateFusionDatasetGraph, self). \
    ...
    def __getitem__(self, idx):
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            lidar_embedding = torch.mean(selected_cav_processed['processed_features']['voxel_features'], dim=0)
                    
            node_feature = {
                'lidar': lidar_embedding,
                'velocity': selected_cav_processed['velocity'],
                'position': torch.tensor(selected_cav_base['params']['lidar_pose'][:2], dtype=torch.float),  # x, y
            }
        ...
        # Create edge indices, labels, and attributes
        num_nodes = len(node_features)
        for i in range(num_nodes):
            for j in range(num_nodes):
                if i != j:
                    edge_indices.append([i, j])
                    # Determine edge label based on distance
                    pos_i = node_features[i]['position']
                    pos_j = node_features[j]['position']
                    ...
                    t_matrix = pairwise_t_matrix[i][j]
                    t_matrix_flat = t_matrix.flatten()
                    edge_attr = torch.cat([torch.tensor(t_matrix_flat, dtype=torch.float), distance.view(1)])
        ...
        print(data)
        processed_data_dict['ego'].update({'graph_data': data})
    ...
    def collate_batch_train(self, batch):
        ...
        graph_data_list = [sample['ego']['graph_data'] for sample in batch]
        batched_graph = torch_geometric.data.Batch.from_data_list(graph_data_list)

        output_dict['ego'].update({'batched_graph': batched_graph})
